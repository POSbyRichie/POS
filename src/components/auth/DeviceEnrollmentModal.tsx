        ...
        <form onSubmit={handleEnrollSubmit} className="p-4 sm:p-6 space-y-4 overflow-y-auto flex-1">
          {!isOnline && (
            <div className="p-3 bg-amber-950/50 border border-amber-800/80 rounded-xl flex items-start gap-2.5 text-xs text-amber-300">
              ...
              <div>
                <span className="font-bold block">Offline Standalone Mode</span>
                Working without internet. This terminal will initialize locally in IndexedDB and synchronize hardware telemetry when connected.
              </div>
            </div>
          )}
          ...
          <div className="p-3.5 bg-sky-950/40 border border-sky-800/50 rounded-xl flex items-center justify-between gap-3">
            <div className="min-w-0">
              <p className="text-xs font-bold text-sky-200">Recommended Fast Setup</p>
              <p className="text-[11px] text-slate-400 truncate">Pair with Register 01 (Main Counter) in 1-click</p>
            </div>
            <button
              type="button"
              disabled={isSubmitting}
              onClick={handleQuickAutoEnroll}
              className="px-3.5 py-2 bg-sky-600 hover:bg-sky-500 active:scale-95 text-white font-bold text-xs rounded-xl shadow-md shadow-sky-600/30 flex items-center gap-1.5 transition shrink-0 cursor-pointer disabled:opacity-50"
            >
          ...
          <div className="p-3 bg-slate-950 rounded-xl border border-slate-800 space-y-1 text-[11px] text-slate-400">
            <div className="flex items-center gap-1.5 text-slate-300 font-semibold">
              ...
              <span>Offline-First Terminal Architecture</span>
            </div>
            <p>1. Terminal binds securely to register instance in local IndexedDB.</p>
            <p>2. Sales and cash management work 100% offline without network reliance.</p>
            <p>3. Automatic background synchronization activates whenever internet is available.</p>
          </div>
          ...
          >
            ...
            <span>{isSubmitting ? 'Enrolling Terminal...' : 'Authorize Terminal & Proceed to POS'}</span>
          </button>
        </form>
      </div>
    </div>
